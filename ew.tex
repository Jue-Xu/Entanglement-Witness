...
\begin{definition}[Schmidt coefficient/rank/measure]
	...
	Schmidt measure is minimum of log r where r is number of terms in an expansion of the state in product basis.
\end{definition}
\begin{example}
...
\begin{definition}[fully separable]\label{def:fully_separable}
	...
	% \end{equation}
	P-bi-separable... $\separable_f^P \subset S_b^P$
\end{definition}
...
\begin{proposition}
	...
	the \nameref{def:entanglement_witness} operator $\ew_{\psi}$ can witness (\nameref{def:genuinely_entangled}) genuine multipartite entanglement near $\ket{\psi}$
...
\begin{definition}[fidelity]\label{def:fidelity}
	...
	Uhlmann fidelity $F(\rho,\rho') : = \Tr(\sqrt{\sqrt{\rho}\rho'\sqrt{\rho}})$
	% \begin{equation}
	% 	F(\ket{\psi},\ket{\psi'}) :=
	% \end{equation}
	% \begin{equation}
...
\begin{problem}[trace estimation]\label{prm:trace_estimation}
	...
	\begin{itemize}
		...
		\item \textbf{Output}: the expectation value $\expval{\ob}=\Tr(\ob \dm) $ with error $\epsilon$ in trace \nameref{def:distance}. \nameref{def:entanglement_witness}, linear function
		\item \textbf{Output}: \nameref{def:fidelity} $F(\dm,\dm')$, \nameref{def:distance}
		\item \textbf{Output}: quantum \nameref{def:kernel} $\Tr(\dm\dm')$
...
\begin{algorithm}[H]
    ...
    \SetKwInOut{Input}{input}
    \SetKwInOut{Output}{output}
    \Input{(copies of) density matrix $\dm$}
    \Output{determine entangled structure??}
    \BlankLine
    \For{ $i = 1,2, \ldots, m$} {
        $\ew_i$  \tcp*{this is a comment}
        % \tcc{comment in a new line}
    {\Return "separable?"}
    }
    \Return entangled ?
