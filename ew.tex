...
Machine techniques including quantum machine learning \cite{congQuantumConvolutionalNeural2019} models have been widely applied to classification tasks in physics, such as classification of phases and prediction of ground state \cite{carrasquillaMachineLearningPhases2017} \cite{huangProvablyEfficientMachine2022}.
...
While the preparation is never perfect, it is still expected that the prepared mixed state is in the proximity of $\ket{\psi_\target}$. 
	Different from \nameref{prm:separability} problem, we assume prior knowledge of the target state $\ket{\psi}$,
	that is, $\ket{\psi_{\target}}$ undergoes some noise channels: white noise, bit/phase-flip error, or random local unitary
...
It can also detect many entangled 2-qubit states.
...
% \begin{definition}[CHSH inequality]\label{def:chsh_inequality}
	...
	CHSH operator $\ew_{\chsh}:=\vb{w}_{\chsh}\cdot\hat{\vb{p}}$ with 
...
In the past several years, classical machine learning algorithms have been proposed for classification tasks related to entanglement.
...
In other word, we derive an entanglement witness for desired states with entannglement structure by fitting a dataset.
...
SVM has the very similar structure to \nameref{def:entanglement_witness}.
...
\begin{table}[!ht]
	...
	\begin{tabular}{c|c|c|c}
		...
		\hline
	\end{tabular}
	\caption{Comparison of CHSH inequality, conventional fidelity witness, and machine learning witness ansatz.}
\end{table}
...
Rigorous analysis \cite{haahSampleoptimalTomographyQuantum2017} \cite{odonnellEfficientQuantumTomography2016} proved that $\Omega(Dr/\epsilon^2)/\log(D/r\epsilon)$ copies/measurements is required for $D$-dimensional ($D=2^n$ for $n$-qubit systems) rank-$r$ density matrix with error $\epsilon$ (trace distance) if adaptive measurements allowed 
...
The answer is yes.
In quantum mechanics, interesting properties are often linear functions of the underlying density matrix $\dm$, such as classical features $\expval{\ob}=\Tr(\ob \dm) $ for entanglement witness
...
This enables the possibility to shadow tomography, the idea proposed by Aaronson \cite{aaronsonShadowTomographyQuantum2018}.
...
In our pipeline, we focus on classical shadow method.
