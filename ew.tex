...
Machine techniques have been widely applied to classification tasks in physics [][].
...
Another widely used one is the k-symmetric extension hierarchy \cite{navascuesPowerSymmetricExtensions2009}, which is presently one of the most powerful criteria, but hard to compute in practice due to its exponentially growing complexity with $k$.[??]
...
\begin{problem}[Entanglement detection]\label{prm:entanglement_detection}
	...
	\begin{itemize}
		\item \textbf{Input}: an unknown state $\dm$ (from experiments) is promised in `vicinity' (undergo noise channels: white noise, bit/phase-flip error, or random local unitary) of a target $\ket{\psi}_{\target}$

		\item \textbf{Output}: $\dm$ possesses `useful' \textsf{entanglement} 
		% ??? $\separable_f^\ppartition$ ? $\separable_b^\ppartition$ 
		(GME, full entanglement, $\separable_b^\ppartition$, intactness, depth ...) or not 
	\end{itemize}
...
Weilenmann et. al \cite{weilenmannEntanglementDetectionMeasuring2020} proposed the idea of unfaithful states which systematically (2-qudit) analyze entangled state with noise cannot be detected by fidelity witness.
...
\begin{problem}[Learning an entanglement witness]
	specific entanglement structures 
	\begin{itemize}
		\item \textbf{Input}:  synthetic data consist of density matrices $\dm$ with corresponding labels $y$
		\item \textbf{Output}: the classifier $\vbw_{\ml}$ with high training accuracy and minimal features $\vbx:=$
	\end{itemize}
\end{problem}
...
% \item 
% \end{itemize}
However, these SVM algorithms cannot be directly applied to experiments.
...
\begin{table}[!ht]
	...
	\begin{tabular}{c|c|c|c}
		...
		SVM witness &  $\ll 4^n-1$ & trained & partial \\  
		\hline
	\end{tabular}
	\caption{ansatz}
\end{table}
...
\begin{algorithm}[H]
    ...
    \SetKwInOut{Input}{input}
    \SetKwInOut{Output}{output}
    \Input{dataset, states with labels $\qty{(\dm_i,y_i)}^m$}
    ...
    \For{ $j = 1,2, \ldots, 4^n$} {
		\While{  accuracy not high enough} {
			randomly select $i$ features $\tilde{\vbx}$ from $\vbx$ \\
			% kernel estimation \tcp*{classical kernel}
			accuracy, classifier = SVM($\qty{(\tilde{\vbx}_j,y_j)}^m$)
			% \tcp*{train SVM}
			% \tcc{comment in a new line}
		}
...
\begin{problem}[shadow tomography]\label{prm:shadow_tomography}
	...
	\begin{itemize}
		\item \textbf{Input}: copies of an unknown $N$-dimensional state $\rho$, $M$ known 2-outcome measurements $E_1,\dots,E_M$
...
\begin{theorem}[\cite{aaronsonShadowTomographyQuantum2018}]\label{thm:shadow_tomography}
	It is possible to do \nameref{prm:shadow_tomography} using $\tilde{\bigO}(\log^4 M\cdot \log N\cdot\epsilon^{-4})$ copies.
...
Inspired by Aaronson's shadow tomography, Huang et. al \cite{huangPredictingManyProperties2020} introduce classical shadow method that is more friendly to experiments.
...
\begin{definition}[classical shadow]\label{def:classical_shadow}
	Given a quantum state $\dm$, its classical shadow (snapshots) $\dm_{cs}$ is 
	\begin{equation}
...
\begin{algorithm}[H]
    ...
    \SetKwInOut{Input}{input}
    \SetKwInOut{Output}{output}
    \Input{samples of $\dm$ and $\ew_{\ml}:=\vb{P}\cdot\vbw_{\ml}$}
	% (black-box access to a circuit preparing a state)
	% , observables $\ob$...
    \Output{estimation of $\Tr(\ew_{\ml} \dm )$}
	...
	\Return estimate $\Tr(\ew\dm)$ 
	\tcp{estimate features for SVM}
    \caption{Estimate features by classical shadow}
...
Derandomization \cite{huangEfficientEstimationPauli2021} can and should be viewed as a refinement of the original classical shadows idea, but not necessarily guarantees better performance for global observables.  
...
\begin{table}[!ht]
	...
	\begin{tabular}{c|c}
		...
		\nameref{def:classical_shadow} & \cref{thm:classical_shadow}  \\
...
% 		% \caption{PPT criterion (2-qubit random density matrix)}
% 	\end{subfigure}
% 	\caption{Data praparation: (a) Bell state with white noise, (b) random 2-qubit density matrix, (c) 3-qubit W state with white noise (d) 3-qubit GHZ state with white noise (e) GHZ state with coherent noise (f) 4-qubit linear cluster PPT. Faithfulness of random 2-qubit states: ...}
% \end{figure}
...
\begin{itemize}
	\item 2-qubit: bipartite $\rho_A\otimes \rho_B$ where $\rho_A$ is a random density matrix sampled (Haar measure)
	\item white noise \cref{eq:white_noise}, coherent noise \cref{eq:coherent_noise}
...
general classical shadow experiments \cite{zhangExperimentalQuantumState2021};
evaluate p3-PPT by classical shadow \cite{elbenMixedstateEntanglementLocal2020}.
