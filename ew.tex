...
So, for practical purpose, it is essential to benckmark (characterize) entanglement (structures) of certain target states in actual experiments.
...
\begin{definition}[fully separable]\label{def:fully_separable}
	An $n$-qubit pure state $\ket{\psi_f}$ is \emph{fully separable} if and only if it can be written in the tensor product form, i.e., $\ket{\psi_f}=\otimes_i^n \ket{\phi_{A_i}}$.
	Analoguous to \cref{eq:mixed}, an $n$-qubit mixed state $\dm_f$  is fully separable if it can be written as a convex combination of pure full separable states.
...
\begin{definition}[bipartite separable]\label{def:bipartite_separable}
	...
	A pure state $\ket{\psi}$ is \emph{bipartite (bi-)separable} if it is in a tensor product form 
	...
	\begin{equation}
		...
		\label{eq:mixed}
	\end{equation}
	The set of all bi-separable states is denoted as $S_b$.
...
\end{definition}
Note that $\ket{\psi_i}$ in \cref{eq:mixed} may be bi-separable with respect to different partitions (otherwise \nameref{def:gme}).
...
\begin{theorem}[PPT criterion]\label{thm:ppt}
	...
	Equivalently, if the smallest eigenvalue of partial transpose $\dm_{AB}^{\T_A}$ is negative, then the state is (fully?) entangled.
...
\begin{theorem}[\cite{gurvitsClassicalDeterministicComplexity2003}]
	...
	\nameref{prm:weak_membership problem_for_separability} is \nameref{def:np}-Hard for $\epsilon=1/\poly(D)$ (with respect to Euclidean \nameref{def:norm} and trace norm).
...
\begin{problem}[Entanglement detection]\label{prm:entanglement_detection}
	...
	\begin{itemize}
		\item \textbf{Input}: a (unknown, experiment) state $\dm$ in the `vicinity' of $\ket{\psi}$ (undergo noise channels: white noise, bit/phase-flip error, random local unitary);

		\item \textbf{Output}: whether $\dm$ has `useful' \textsf{entanglement} or not 
		% ??? $\separable_f^\ppartition$ ? $\separable_b^\ppartition$ 
		(GME, full entanglement, $\separable_b^\ppartition$, intactness, depth ...)
	\end{itemize}
...
This shows that ﬁdelity-based entanglement witnesses detect entanglement that is useful.
...
\begin{algorithm}[H]
    ...
    \SetKwInOut{Input}{input}
    \SetKwInOut{Output}{output}
    \Input{samples of $\dm$ and $\ew_{\ml}:=\vb{P}\cdot\vbw$}
