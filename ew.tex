...
but this line of work is still based on \nameref{thm:ppt} (not iff).
...
\end{equation}
is larger than 1/2.
We can see in \cref{fig:unfaithfulness}, even for 2-qubit states, nonnegligible portion of randomly sampled states are unfaithful but still entangled (NPT).
\begin{figure}[!ht]
	\centering
	\includegraphics[width=.9\linewidth]{./Code/faithfulness_2_qubit.png}
	% \caption{PPT criterion (2-qubit random density matrix)}
	\caption{Unfaithfulness of 2-qubit states: $10^4$ random 2-qubit states are categorized according to the minimal eigenvalue of partial transpose $\dm_{AB}^{\T_A}$ and the maximal eigenvalue of $\chi_2(\dm_{AB})$.}
...
\begin{figure}[!ht]
	...
	\caption{Schematic diagram for entanglement detection methods: entanglement witnesses for different states are depicted by colored dash lines (hyperplane). SVM with the linear kernel (hyperplane). \nameref{thm:ppt} (non-linear, one-side) ... }
	\label{fig:entangle}
\end{figure}
...
find a hyperplane $(\vbw,b)$ (a linear function) such that maximize the margin between two partitions (see \cref{fig:entangle})
\begin{equation}
	\max_{\vb{w}}
	\norm{\vb{w}}_2^2
	\text{ s.t. }
...
% \end{equation}
So, the prediction label is given by the sign of the inner product (projection) of the hyperplane and the feature vector, $y=\text{sign}(\vbw\cdot\vbx+b)$.
...
Both SVM witness and conventional fidelity witness, i.e., $\Tr(W\dm)\equiv\expval{\ew}=\vbw \cdot \vbx$, is a weighted sum of observables (features) which represents a hyperplane in the state (feature) space, see \cref{fig:entangle}.
The SVM witness is more flexible because coefficients are optimized (automatically derived) from training for any generic target state.
...
A key drawback of conventional witnesses is its linearity because many real-world datasets are not linearly-separable in low dimensional feature space.
Despite the nonlinear witness \cite{guhneNonlinearEntanglementWitnesses2006} proposed, its implementation in experiments is more challenging than linear ones.
The good news is,
...
The main idea of the kernel method is mapping the features to a higher dimensional space via a feature map $\phi(\vbx)$ such that they can be linearly separated in the high dimensional feature space.
...
\cref{fig:feature_space} exhibits that two kinds of data points are clearly classified by nonlinear SVM, though it is not linearly separable in the 2-dimensional space.

We focus on kernel methods rather than neural networks that is also non-linear, 
...
In our protocol, we focus on the classical shadow method and its variants.
...
\end{figure}
For the machine learning part, we make use of scikit-learning package \cite{pedregosaScikitlearnMachineLearning2011} to train SVM with RBF kernel.
...
if we have all classical features, is it possible to train a universal classifier or with weaker promise;
