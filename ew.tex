...
\begin{theorem}[PPT criterion \cite{horodeckiSeparabilityMixedStates1996}]\label{thm:ppt}
	...
	Equivalently, if the smallest eigenvalue of partial transpose $\dm_{AB}^{\T_A}$ is negative, then the state is entangled.
	Note that PPT is a necessary and sufficient condition for \nameref{prm:separable} when $d_A d_B \le 6$.
\end{theorem}
...
\begin{remark}
	...
	Unconscious phase accumulation $\ket{\psi_\phi}=\frac{1}{\sqrt{2}}\qty(\ket{000}+e^{\ii \phi}\ket{111})$;
	Rotation on the first control qubit $\ket{\psi_\phi^\theta}=\frac{1}{\sqrt{2}}\qty(\cos\theta\ket{000}+e^{\ii \phi}\sin\theta\ket{111})$
\end{remark}
% \begin{figure}[!ht]
% 	\centering
...
\begin{definition}[entanglement witness]\label{def:entanglement_witness}
	Given a speific quantum state $\dm$ (density matrix), its \emph{entanglement witness} $\ew$ is an obseverable such that
	\begin{equation}
		\expectation_{\dm}[\ew]\equiv
		\expval{\ew}\equiv
		\Tr(\ew\dm) \ge 0 , \forall \text{ separable };\quad
		\Tr(\ew\dm) < 0 , \text{ for some entangled including } \rho
	\end{equation}
...
\begin{table}[!ht]
	...
	\begin{tabular}{c|c|c|c}
		...
		SVM witness & ?? & trained & partially known \\  
		\hline
	\end{tabular}
	\caption{ansatz}
\end{table}
...
\begin{algorithm}[H]
	...
	\tcc{predict}
	\tcp*{============================================= testing phase =============================================== //}
    \caption{witness ansatz + classical shadow (..) + classical machine learning (\nameref{def:svm}) }
