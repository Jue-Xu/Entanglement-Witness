...
This paper is organized as follows: \cref{sec:preliminaries} presents the definitions about entanglement and several mainstream entanglement detection methods;
\cref{sec:protocol} demonstrates our end-to-end protocol including machine learning part and expectation estimation part;
at last, numerical simulation and results are discussed in \cref{sec:numerical_simulation}.
...
\begin{definition}[fully entangled]\label{def:fully_entangled}
	A state $\dm$ is a fully entangled
...
For a general review on entanglement detection, we refer readers to \cite{guhneEntanglementDetection2009}.
...
\begin{theorem}[PPT criterion]\label{thm:ppt}
	...
	By contrapositive, if the smallest eigenvalue of partial transpose $\dm_{AB}^{\T_A}$ is negative, then the state is entangled (cannot be bi-separable) with respect to the partition $\ppartition=\qty{A,B}$.
	PPT is a necessary and sufficient condition for \nameref{prm:separability} when $d_A d_B \le 6$ \cite{horodeckiSeparabilityMixedStates1996}.
\end{theorem}
Restate in theoretic CS language
...
\begin{theorem}[\cite{gurvitsClassicalDeterministicComplexity2003}]
	...
	% \end{itemize}
\end{theorem}
Even we know the complete information about a general state, 
it is hard to determine its separability.
...
This is a key distinction from entanglement witnesses, which can be more powerful, but which \textbf{usually require detailed prior information about the state}.
...
\begin{definition}[entanglement witness]\label{def:entanglement_witness}
	Given a speific entangled state $\ket{\psi}_{\target}$, its entanglement witness $\ew$ is an obseverable such that
	\begin{equation}
...
\end{equation}
Within some noise parameter regime, the `noisy' GHZ state cannot be detected by conventional fidelity witness (need post measurement processing).
\begin{figure}[!ht]
	\centering
...
which systematically analyze entangled (2-qudit) state with noise cannot be detected by fidelity witness.
...
The brute force approach is to fully characterize a state by performing \emph{quantum state tomography} and calculating classcial features or separability measures from the recovered density matrix.
\begin{problem}[quantum state tomography]\label{prm:full_tomography}
	Informally, quantum state tomography refers to the task of estimating complete description (density matrix) of an unknown target $D$-dimensional quantum mixed state $\dm$ within some error, 
	...
	\end{equation}
	where $\bm{\sigma}\in \qty{\identity,\sx,\sy,\sz}^n$.
\end{problem}
...
Intuitively, a general tomography \cite{altepeterPhotonicStateTomography2005} that extract (recover) all information of a state requires exponential of $n$ copies (measurements?).
...
% 	We rigorously show that, for any quantum process $\mathcal{E}$, observables $\ob$, and distribution $\mathcal{D}$, and for any quantum ML model, one can always design a classical ML model achieving a similar average prediction error such that $N_C$ (number of experiments?) is larger than $N_Q$ by at worst a small polynomial factor.
	...
	In contrast, for achieving accurate prediction on all inputs, we prove that \textbf{exponential quantum advantage is possible}. For example, to predict expectations of \textbf{all Pauli observables} in an $n$-qubit system $\dm$, classical ML models require $2^{\Omega(n)}$ copies of $\dm$ , but we present a quantum ML model using only $\bigO(n)$ copies.
...
% 		\includegraphics[width=.9\linewidth]{./Code/faithfulness_2_qubit.png}
	% \caption{PPT criterion (2-qubit random density matrix)}
	\caption{Data praparation: (a) Bell state with white noise, (b) random 2-qubit density matrix, (c) 3-qubit W state with white noise (d) 3-qubit GHZ state with white noise (e) GHZ state with coherent noise (f) 4-qubit linear cluster.}
\end{figure}
