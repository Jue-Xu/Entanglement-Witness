...
\begin{abstract}
	...
	In this protocol, an entanglment witness for a generic entangled state is trained by classical SVM with synthetic data which consist of classical features of quantum state and their labels. 
	In real experiments, classical features of a state, that is expectation values of a set of  Pauli measurements, are estimated by sample-efficient methods such as classical shadow.
...
However, decoherence is inevitable in real-world, which means the interaction between a quantum system and classical environment would greatly affect entanglement quality and diminish quantum advantage. 
So, for practical purpose, it is essential to benchmark (characterize) entanglement (structures) of certain target states in actual experiments.
Machine techniques have been widely applied to classification tasks in physics \cite{carrasquillaMachineLearningPhases2017} \cite{huangProvablyEfficientMachine2022}.
...
Assume we would like to distinguish an entangled state incluing its `vicinity' from all separable states, our method derive such a classifier by fitting a dataset sampled from target states with their labels ($\entangled$ or not).
Specifically, our pipeline starts from evaluation of expectations of a subset of all possible $n$-qubit pauli operators. 
The set of expectation values that serves as classical features of a quantum state, together with its label, consist of a data point.
A classical machine learning classifier, SVM \cite{cortesSupportvectorNetworks1995} is obtained by training with this dataset.
...
This paper is organized as follows: in \cref{sec:preliminaries}, we presents necessary definitions about entanglement structures and mainstream entanglement detection methods;
\cref{sec:protocol} demonstrates our end-to-end protocol including two parts: learning an entanglement witness and estimating classical features;
at last, numerical simulation results and comparison are discussed in \cref{sec:numerical_simulation}.
...
Large scale entanglement involving multiple particles maybe the main resource of quantum advantages in quantum computation and communication.
Roughly, we say a quantum state is entangled if it is not fully separable.
\begin{definition}[fully separable]\label{def:fully_separable}
	An $n$-particle (qubit) pure state $\ket{\psi_f}$ is fully separable 
	if it can be written as the tensor product of subsystems $\qty{A_1,\dots,A_n}$, i.e., $\ket{\psi_f}=\bigotimes_{i=1}^n \ket{\phi_{A_i}}$.
	% Analoguous to \cref{eq:mixed}, 
	Then, a mixed state $\dm_f$  is fully separable if it can be written as a convex combination of fully separable pure states.
...
On the other hand, if a state is not a convex combination of any (partition) biseparable states,
it means that all qubits in the system are indeed entangled with each other.
This is the strongest form of entanglement, called genuine multipartite entanglement, formally defined as
...
In contrast to $S_{\bi}$, a state is $\ppartition_2$-separable, if it is a mixing of pure separable states with a same partition $\ppartition_2$, 
and we denote the state set as $S_{\bi}^{\ppartition_2}$. 
It is practically interesting to study entanglement structure under partitions,
because bi-separability between two subsystems naturally indicates loss of quantum information processing capabilities among certain geometric configuration in quantum commnuication or computation.
...
\begin{definition}[full entanglement]\label{def:full_entanglement}
	...
	if it is outside of the separable state set $S_{\bi}^{\ppartition_2}$ for any partition,
	that is, $\forall \ppartition_2 = \qty{A,\bar{A}},\dm \notin S_\bi^{\ppartition_2}$.
	% \begin{equation}
...
so full entanglement is weaker than GME.
...
\subsection{Entanglement detection}
Entanglemtn detection for a general state is a highly non-trivial problem.
...
\end{problem}
% \nameref{thm:ppt}
It is not hard to prove that if a state is \nameref{def:bipartite_separable}, then it must have positive partial transpose (PPT), 
...
\begin{theorem}[PPT criterion]\label{thm:ppt}
	...
	If the smallest eigenvalue of partial transpose $\dm_{AB}^{\T_A}$ is negative, then the state is entangled (cannot be bi-separable) with respect to the partition $\ppartition=\qty{A,B}$.
\end{theorem}
We should notice that PPT is a necessary and sufficient condition for \nameref{prm:separability} for low-dimensional systems 
...
By relaxing the defintion (promise a gap), a reformulation of this problem in the theoretic CS language is
...
\begin{theorem}[\cite{gurvitsClassicalDeterministicComplexity2003}]
	...
	% \end{itemize}
\end{theorem}
Even we know the complete information about a general state, it is hard to determine its separability.
However, it does not rule out the possibility to solve it efficiently with stronger promise (approximation) or quantum algorithms, even machine learning powered by data.
...
In order to apply the \nameref{thm:ppt}, the full density matrix must be available. However, \nameref{prm:full_tomography} requires an exponential number of measurements.

Similar to the PPT condition, the $p_3$-PPT condition applies to mixed states and is completely independent of the state in question. 
...
This is a key distinction from entanglement witnesses, which can be more powerful, but which usually require detailed prior information about the state.
\begin{problem}[entanglement detection]\label{prm:entanglement_detection}
	...
	Different from \nameref{prm:separability} problem, we assume prior knowledge of the target state $\ket{\psi}$
	\begin{itemize}
		\item \textbf{Input}: an unknown state $\dm$ (from experiments) is promised in `vicinity' of a target $\ket{\psi}_{\target}$, that is, $\ket{\psi}_{\target}$ undergoes some noise channels: white noise, bit/phase-flip error, or random local unitary

		\item \textbf{Output}: $\dm$ possesses `useful' entanglement
		% ??? $\separable_f^\ppartition$ ? $\separable_b^\ppartition$ 
...
\begin{definition}[entanglement witness]\label{def:entanglement_witness}
	Given a speific entangled state $\ket{\psi_\target}$, its entanglement witness $\ew$ is an obseverable such that
	\begin{equation}
...
In a typical experiment one aims to prepare a pure state, $\ket{\psi_\target}$, and would like to detect it as true multipartite entangled. 
While the preparation is never perfect, it is still expected that the prepared mixed state is in the proximity of $\ket{\psi_\target}$. The usual way to construct entanglement witnesses using the knowledge of this state is
\begin{equation}
...
For graph states (stabilizer states),
...
The common robustness measure of fidelity wintess is the tolerence of white noise.
\begin{equation}\label{eq:white_noise}
	\dm'
	= (1-p_{\noise}) \op{\psi_{\target}} + 2^{-n} \cdot p_{\noise} \identity
\end{equation}
...
For non-stabilizer case, more careful analysis is required \cite{zhangEfficientEntanglementGeneration2021} \cite{zhuMachineLearningDerivedEntanglement2021}.
...
For GHZ and W state with white noise, we can analytically compute the white noise threshold for the \nameref{thm:ppt} (NPT for bipartite entanglement).
When $p_{\noise}<0.8$, GHZ states have \nameref{def:full_entanglement}.
However, the conventional fidelity witness detects \nameref{def:gme} when $p_{\noise}<1/2$.
It would be practically interesting to have a witness for full entanglement.

Other than white noise, more practical noise happened in experiments is coherent noise, e.g., local unitary/rotation.
	% other noise (depolarization)? e.g., flip error, phase error?, local, random unitary transformation?
Take GHZ as an example, unconscious phase accumulation and 
...
which systematically analyze entangled (2-qudit) state mixed with white noise cannot be detected by fidelity witness.
...
\begin{definition}[unfaithful state]\label{def:unfaithful_state}
	...
	\end{equation}
	if and only if the smallest eigenvalue of $X_2( \dm_{AB})$ is greater than 1/2.
\end{definition}
...
\begin{figure}[!ht]
	...
	\caption{Faithfulness of random 2-qubit states: ...}
\end{figure}
...
classical machine learning (SVM, NN) \cite{huangProvablyEfficientMachine2022}??: classify phase, predict ground state, entanglement?
There are quantum machine learning algorithms for quantum problems (data) \cite{congQuantumConvolutionalNeural2019}
...
Different from \nameref{prm:entanglement_detection}, we assume training data
...
It is not surprised that tomographic ansatz has better performance (also required \cite{luTomographyNecessaryUniversal2016}) for universal bi-separability.
However, training a universal classifier (or GME) for multi-qubit, high-dimensional system is hard if the gap between two sets is small.
...
On the other hand, \nameref{prm:entanglement_detection} is also studied by classical machine learning algorithm \cite{zhuMachineLearningDerivedEntanglement2021}, but a different technique called Support Vector Machine (\nameref{def:svm}).
SVM has the very similar structure as \nameref{def:entanglement_witness}.
...
An SVM allows for the construction of a hyperplane $\expval{\ew}=\sum_k w_k \vbx_{k}$ that clearly delineates between separable states and the target entangled state 
...
We focus on kernel methods rather than neural networks, as they not only provide provable guarantees, but are also very flexible in the functions they can learn. 
...
\begin{table}[!ht]
	...
	\begin{tabular}{c|c|c|c}
		...
		\hline
	\end{tabular}
	\caption{conventional fidelity witness, machine learning witness ansatz}
\end{table}
...
but we cannot directly evaluate them as in numerical simulation because we do not know density matrices of incoming states.
...
\begin{problem}[quantum state tomography]\label{prm:full_tomography}
	Informally, quantum state tomography refers to the task of estimating complete description (density matrix) of an unknown $D$-dimensional mixed state $\dm$ within error tolerence, 
...
Since the density matrix has $D^2$ entries, intuitively, a general tomography \cite{altepeterPhotonicStateTomography2005} requires $(D^2)?$ copies (independent measurements?).
...
Given an observable (Hermitian) $\ob$ and (copies of) a mixed state $\dm$ or several states ($\dm',\dots,\dm_m$), 
to estimate linear functions $\expval{\ob}=\Tr(\ob \dm) $, entanglement witness;
nonlinear functions: \nameref{def:entropy};
...
This enables the possibility to shadow tomography \cite{aaronsonShadowTomographyQuantum2018}.
...
% \begin{theorem}[lower bound of \nameref{prm:full_tomography}?\cite{haahSampleoptimalTomographyQuantum2017}]
	Known fundamental lower bounds state that classical shadows of exponential size (at least) $T = \Omega( 2^n / \epsilon^2)$ are required to $\epsilon$-approximate $\dm$ in trace \nameref{def:distance}.
...
% \end{theorem}
Inspired by Aaronson's shadow tomography, Huang et. al \cite{huangPredictingManyProperties2020} introduce classical shadow (CS) method that is more friendly to experiments.
...
\begin{theorem}[?]\label{thm:classical_shadow}
...
Derandomized variant of classical shadow \cite{huangEfficientEstimationPauli2021} is the refinement of the original randomized protocol, 
...
\begin{table}[!ht]
	...
	\begin{tabular}{c|c}
		...
		classical shadow & \cref{thm:classical_shadow}  \\
		...
		\cref{thm:quantum_ml_estimate_bound} (Q. advantage)\\  
		\hline
	\end{tabular}
...
multi-partite entangled states generation: Bell, GHZ, W state, graph (cluster) state
% \begin{equation}
...
random multi-qubit pure states: $\dm_A\otimes \dm_{BC}$, $\dm_C\otimes \dm_{AB}$, and $\dm_B\otimes \dm_{AC}$.
noice channels: white noise \cref{eq:white_noise}, coherent noise \cref{eq:coherent_noise}.
dataset size: $10^4$ for 3-qubit case.
...
experiments of classical shadow \cite{zhangExperimentalQuantumState2021};
detect entanglement by estimating $p_3$-PPT by classical shadow \cite{elbenMixedstateEntanglementLocal2020}.
