...
\begin{problem}[entanglement detection]\label{prm:entanglement_detection}
	...
	or (ii) in proximity of a target $\ket{\psi_{\target}}$ (possesses `useful' entanglement such as \nameref{def:gme}, \nameref{def:full_entanglement}, depth ...) 
	\footnote{
...
\begin{algorithm}[H]
	...
	% \tcp*{---------------------------------------------- training phase ------------------------------------------------}
	% \tcc{training phase}
	Evaluate all Pauli observables $\vbx^{(i)}:=\Tr(\pob_{\vbx}\dm^{(i)})$,  $\forall i$ \\
    \For{ $j = 1,2, \ldots, 4^n$} {
...
\begin{algorithm}[H]
    ...
    \SetKwInOut{Input}{input}
    \SetKwInOut{Output}{output}
    \Input{samples of $\dm$ and $\pob_{\vec{\bmsigma}_\ml}$}
	% (black-box access to a circuit preparing a state)
	% , observables $\ob$...
    \Output{estimation of $\vbx_{\vec{\bmsigma}_{\ml}}:=\Tr(\pob_{\vec{\bmsigma}_{\ml}}\dm)$}
