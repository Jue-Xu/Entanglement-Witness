...
% \end{figure}
It worth noting that 2D cluster (2D rectangular lattice graph) state is the universal resource for the measurement based quantum computation (MBQC) \cite{briegelMeasurementbasedQuantumComputation2009}.
...
\begin{table}[!ht]
	...
	\begin{tabular}{c|c|c|c}
		...
		\nameref{def:entanglement_witness} & convex?\cite{chakrabartiQuantumAlgorithmsLower2020}  \cref{thm:entanglement_witness_gme} (constant?)& fixed & known  \\  
		Bell (CHSH) inequality & & fixed & unknown \\  
		entangle spectrum \cite{horodeckiDirectDetectionQuantum2002} & & & unknown \\  
		ML/tomographic witness & & trained & unknown \\  
		\hline
	\end{tabular}
	\caption{ansatz}
\end{table}
