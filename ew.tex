...
%	In this protocol, an entanglement witness for a generic entangled state is obtained by classical machine learning with a synthetic dataset which consists of classical features of certain states and their labels. 
...
%but more analysis and measurement settings are required for more general states and noise cases \cite{guhneNonlinearEntanglementWitnesses2006} \cite{zhouEntanglementDetectionCoherent2020} .
...
%where classical features of quantum states are estimated by the classical shadow method \cite{huangPredictingManyProperties2020} with affordable sample complexity.
...
(1) the fidelity $ \Tr(\dm_{\prepare}\op{\psi_{\target}}) < \alpha$ implies $\dm\in\separableset_{\bi}$ or entangled;
(2) the fidelity $ \Tr(\dm_{\prepare}\op{\psi_{\target}}) \ge \alpha$ implies $\dm\notin\separableset_{\bi}$
(In other words, the trace distance $\norm{\dm_{\prepare}-\op{\psi_\target}}_1 \le \sqrt{1-\alpha}$)  \footnote{The fidelity and trace distance are related by the inequalities
...
The SVM formalism allows for the programmatic elimination of features \cite{guyonGeneSelectionCancer2002}, i.e., reducing the cost of experimental measurements and copies (samples).
...
then we randomly eliminate one feature such that the training accuracy keeps high.
By repeating this procedure, we finally obtain a classifier with minimal number of features.
...
\begin{algorithm}[H]
	...
	% \tcp*{---------------------------------------------- training phase ------------------------------------------------}
	% \tcc{training phase}
	Evaluate $k$-local Pauli observables $\vbx^{(i)}:=\Tr(\dm^{(i)}\pob_{\bmsigma}), \forall i$ \\
...
\begin{table}[!ht]
	...
	\begin{tabular}{c|c|c|c}
		...
		Conventional fidelity  witness & few LMS & fixed & strong  \\  
...
\begin{algorithm}[H]
    ...
    \For{ $i = 1,2, \ldots, R$} {
		...
		$\U\dm \U^\dagger\mapsto \ket{b}\in \qty{0,1}^n $ \tcp*{measure in computational basis}
    ...
    \caption{estimate features (local Pauli observables by (randomized) classical feature}
...
% 	\dm_{\cs} := \mathcal{M}^{-1} \qty(U^\dagger \op{b} U),
	\dm_{\cs} := \bigotimes \qty(3U_j^\dagger \op{b_j} U_j-I),
\end{equation}
% where $\mathcal{M}=$.
...
\begin{figure}[!ht]
	\centering
	\includegraphics[width=.9\linewidth]{./Code/estimation_error_compare_methods.png}
	\caption{Average error of estimating expectation values of $M$ 2-local 4-qubit Pauli observables $\sum_i (o_i-\Tr(\ob_i\dm))^2/M$ by different estimation methods VS number of samples (the error bar indicates the standard deviation for each case). The blue line represents estimating each Pauli observables independently. The orange line is the estimation by the randomized classical shadow, while the green one is the derandomized version of classical shadow.}
	\label{fig:shadow}
\end{figure}
...
\footnote{The open-source code for classical shadow \url{https://github.com/hsinyuan-huang/predicting-quantum-properties}}
