...
\begin{equation}
	...
	\label{eq:witness}
\end{equation}
which means that the witness $W$ has a positive expectation value on all separable states, hence a negative expectation value implys the presence of entanglement (GME).
...
but non-local measurements are required for non-stabilizer cases (e.g., W state) \cite{zhangEfficientEntanglementGeneration2021} \cite{zhuMachineLearningDerivedEntanglement2021}.
...
the noise is randomly (uniform) sampled from $p_{\noise}\in[0,0.5]$, $\theta\in[0,\pi/3]$, and $\phi\in[0,0.6\pi]$.
