...
\begin{algorithm}[H]
    ...
    \BlankLine
	Randomly select $R$ Pauli observables from $\qty{X,Y,Z}^n$ 
    ...
    \For{ $i = 1,2, \ldots, R$} {
		...
		$\dm_{\cs}=\bigotimes_j \qty(3U_j^\dagger \op{b_j} U_j-I)$ \tcp*{}
...
\end{algorithm}
Given a quantum state $\dm$ of $n$ qubits,
...
There are several variants of classical shadow method.
The derandomized classical shadow \cite{huangEfficientEstimationPauli2021} is the refinement of the original randomized protocol which provides better performance for $k$-local observables. 
...
The entanglement detection by estimating $p_3$-PPT with classical shadow and comparison with related methods have been done experimentally \cite{elbenMixedstateEntanglementLocal2020} \cite{zhangExperimentalQuantumState2021}.
