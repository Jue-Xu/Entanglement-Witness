...
\begin{theorem}[\cite{aaronsonShadowTomographyQuantum2018}]\label{thm:shadow_tomography}
	It is possible to do \nameref{prm:shadow_tomography} using $\tilde{\bigO}(\frac{\log^4 M\cdot \log D}{\epsilon^4})$ copies. [no construction algorithm?]
...
\begin{remark}
	...
	\begin{itemize}
		...
		\item graph state encoding: \nameref{def:graph_state}, discrete, efficient? space (time)
...
an ansatz
\begin{equation}
	\ew_{a} := \sum_i  a_{..} \bigotimes_i^n \hat{\sigma}^{(i)}
