...
\nameref{prm:weak_membership problem_for_separability} is NP-Hard for $\epsilon=1/\poly(d_A,d_B)$ with respect to Euclidean norm and trace norm \cite{gurvitsClassicalDeterministicComplexity2003} \cite{gharibianStrongNPHardnessQuantum2009},
while there exists a quasipolynomial-time algorithm with respect to the norm $\norm{\cdot}_{\locc}$ and $\norm{\cdot}_2$ \cite{brandaoQuasipolynomialtimeAlgorithmQuantum2011}.
...
The quantum complexity of related separability testing problems were studied in the framework of quantum interactive proofs \cite{gutoskiQuantumInteractiveProofs2015}.
...
It is clear that \nameref{prm:separability} or \nameref{prm:entanglement_detection} problem are exactly such binary classification problems where each quantum state has a binary label, such as either $\entangled$ or \textsf{separable}.
...
We focus on kernel methods rather than neural networks that is also nonlinear, 
...
The advantages of SVM is at least two-fold: (1) the training of an SVM is convex; if a solution exists for the given target state and ansatz, the optimal SVM will be found.
(2) this SVM formalism allows for the programmatic elimination of features \cite{guyonGeneSelectionCancer2002}, i.e., reducing the cost of experimental measurements and copies (samples). 
...
dataset size for training: $10^3$ for 3-qubit case;
...
\begin{figure}[!ht]
	...
	\caption{The states cannot be detected by fidelity witness (GHZ state with coherence noise, and W state with large white noise) can be classified by the kernel SVM classifier with high accuracy.}
...
while the SVM witness can classify them with hight accuracy.
The features $\vbx$ for the kernel SVM $\vbx=(\expval{XXX},\expval{ZZZ},\expval{XXX},\expval{XXX},\expval{XXX})$.
...
(3) quantum machine learning for estimating all classical features and train a tomographic SVM classifier for wider range of states.
