...
\begin{problem}[learning an entanglement witness]\label{prm:learn_witness}
	...
	\begin{itemize}
		...
		\item \textbf{Output}: a learned classifier $f(\vbx_{\dm,\bmsigma})$ with high training accuracy where $\bmsigma$ is a subset of all Pauli observables and $\vbx_{\dm,\bmsigma}$ is a vector of corresponding expectation values.
		% (minimal) 
	\end{itemize}
\end{problem}
...
\begin{algorithm}[H]
    ...
    \Output{a classifier $f(\vbx_{\dm,\bmsigma})$}
    \BlankLine
	% \tcp*{---------------------------------------------- training phase ------------------------------------------------}
	% \tcc{training phase}
	\tcc{evaluate all $k$-local Pauli observables}
	$\vbx^{(i)}:=\Tr(\dm^{(i)}\pob_{\bmsigma}), \forall i$ \\
	\While{\textup{accuracy $< \epsilon$ or} $len(\vbx)>M$ } {
		\For{$j \textup{ in  range}(len(\vbx)) $} {
			...
			\tcc{If cannot find a classifier with less features, then output the last classifier with high accuracy}
				\Return a classifier $f(\vbx_{\dm,\bmsigma})$
			}
		}
	} \Return a classifier $f(\vbx_{\dm,\bmsigma})$
...
The conventional fidelity witness only need few local measurement settings for stabilizer states, but it is a one-side test (required very strong promise measured by fidelity). 
...
permutations $(1,2),(1,3),(1,4),(2,3),(2,4),(3,4)$ and by symmetry of qubits of GHZ and W states....
