...
\begin{definition}[density matrix]\label{def:density_matrix}
	A quantum (mixed) state $\dm$ can be represented by a density matrix which is a Hermitian, \nameref{def:psd} operator (matrix) of trace one. 
	% defined to be a positive operator $ \dm \in \text{End}(V )$ with $\Tr( \dm ) = 1$.
...
\begin{definition}[entanglement entropy]\label{def:entanglement_entropy}
	...
	\end{equation}
	where $\dm_A$ and $\dm_B $ 
	are the \nameref{def:reduced_density_matrix} for each partition.
...
A \nameref{def:stabilizer} $S_i$ is an $n$-fold tensor product of $n$ operators chosen from the one qubit Pauli operators $\qty{\identity,X,Y,Z}$.
\begin{example}[GHZ]\label{exm:ghz}
...
% \subsubsection{quantum kernel}\label{sec:quantum_kernel}
% related works:
% \begin{itemize}
