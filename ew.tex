...
\begin{definition}[maximally entangled]
	a state vector is maximually entangled $\iff$ the reduced state at one qubit is maximally mixed, i.e.,
...
% \begin{question}
	...
	% specific cases? 
	approximately correct complexity? quantum complexity? machine learning (data)? for entanglement () problem
% \end{question}
...
\begin{example}[entanglement witness for GHZ]
	...
	\end{equation}
	$S_k$ is the stabilizer ...
	Detecting Genuine Multipartite Entanglement with Two Local Measurements \cite{tothDetectingGenuineMultipartite2005}
\end{example}

It is natural to ask nonlinear entanglement witness \cite{guhneNonlinearEntanglementWitnesses2006}  and the \nameref{def:kernel} method in machine learning. 
...
\begin{definition}[fidelity]\label{def:fidelity}
	...
	Uhlmann fidelity $F(\dm,\dm') : = \Tr(\sqrt{\sqrt{\dm}\dm'\sqrt{\dm}})$
	% \begin{equation}
	% 	F(\ket{\psi},\ket{\psi'}) :=
	% \end{equation}
	% \begin{equation}
...
\begin{definition}[distance]\label{def:distance}
	trace distance $d_{\tr}(\dm,\dm') : = \frac{1}{2} \norm{\dm-\dm'}_1$
	% \begin{equation}
...
\begin{problem}[trace estimation]\label{prm:trace_estimation}
	...
	\begin{itemize}
		\item \textbf{Input}: Given an observable (Hermitian) $\ob$ and (copies of) a mixed state $\dm$ ($\dm',\dots,\dm_m$), 
		\item \textbf{Output}: with $\epsilon$ in trace \nameref{def:distance}
		\begin{itemize}
...
\begin{remark}[\cite{huangPredictingManyProperties2020}]
	While very efficient in terms of samples, Aaronson's procedure is very demanding in terms of quantum hardware — a concrete implementation of the proposed protocol requires exponentially long quantum circuits that act collectively on all the copies of the unknown state stored in a quantum memory.??
\end{remark}
...
% \begin{remark}
	...
	\begin{itemize}
		...
		need $\log(d) $ qubits for a data point; dequantization \cite{tangQuantumPrincipalComponent2021}. qubit (basic) encoding (inefficient? space)

		\item unitary encoding: quantum simulation (Hamiltonian); quantum random walk (adjacency matrix); oracle (controlled) unitary, quantum phase estimation;
...
\begin{example}[colorable]\label{exm:colorable}
	...
	bipartite graph $\iff$ 2-colorable
\end{example}
...
\begin{algorithm}[H]
    ...
    \Input{a density matrix $\dm$ (many copies), observables $\ob$...}
...
\begin{definition}[divergence]\label{def:divergence}
	...
	Analogously, quantum Jensen-Shannon divergence $D_{\text{QJS}}$ can be defined...
\end{definition}
% \begin{figure}[!ht]
% 	\centering
...
\begin{proposition}[\cite{huangPowerDataQuantum2021}]
	If a classical algorithm without training data can compute $f(x)=\mel{x}{\U_{\textup{QNN}}^\dagger \ob U_{\textup{QNN}}}{x}$ (amplitude encoding) efficiently (poly time in ...) for any $\U_{\textup{QNN}}$ and $\ob$, then $\nameref{def:bpp}=\nameref{def:bqp}$ (which is believed unlikely).
\end{proposition}
\begin{proposition}[\cite{huangPowerDataQuantum2021}]
...
\begin{algorithm}[H]
    ...
    \SetKwInOut{Input}{input}
    \SetKwInOut{Output}{output}
    \Input{(copies of) density matrix (graph state?) $\dm$, an entanglement witness (observable) $\ew$}
    \Output{determine entangled structure??}
    \BlankLine
    \For{ $i = 1,2, \ldots, m$} {
        $\ew_i$  \tcp*{estimate entanglement witness by quantum circuit}
        % \tcc{comment in a new line}
    {\Return $\Tr(\ew\dm)$ }
    }
    \Return entangled ? GME ? separable with certain partition?
    \caption{\nameref{def:entanglement_witness} by ... quantum trace estimation}
...
an ansatz for \nameref{def:entanglement_witness} (graph state entanglement)
\begin{equation}
	\ew_{a} := \sum_{\qty{i\dots}}  w_{..} \bigotimes_i^n \hat{\sigma}^{(i)}
...
\begin{tabular}{c|c|c|c|c}
	...
	\nameref{def:entanglement_witness} & &  \cref{thm:entanglement_witness_gme} (constant?) & convex?& \\  
	classical ML + \nameref{def:classical_shadow}  & ?? & & & \\  
	quantum (variational) circuits &  \cref{thm:multivariate_trace} (c-depth?) & & & \\  
	\hline
\end{tabular}
...
future: experimental implementation with a few qubits (generation, verification) \cite{luEntanglementStructureEntanglement2018}
% \begin{itemize}
